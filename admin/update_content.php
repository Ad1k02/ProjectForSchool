...
if(isset($_POST['update'])){
   ...
   if(!empty($thumb)){
      if($thumb_size > 2000000){
         ...
         if($old_thumb != '' && $old_thumb != $rename_thumb){
            if(file_exists('../uploaded_files/'.$old_thumb)){
   ...
   if(!empty($video)){
      ...
      if($old_video != '' && $old_video != $rename_video){
         if(file_exists('../uploaded_files/'.$old_video)){
...
<section class="video-form">
   ...
   <h1 class="heading">обновить контент</h1>
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <p>обновить название <span>*</span></p>
      <input type="text" name="title" maxlength="100" required placeholder="введите название виде
<input type="text" name="title" maxlength="100" required placeholder="введите название видео" class="box" value="<?= $fecth_videos['title']; ?>">
<p>обновить описание <span>*</span></p>
<textarea name="description" class="box" required placeholder="напишите описание" maxlength="1000" cols="30" rows="10"><?= $fecth_videos['description']; ?></textarea>
<p>обновить плейлист</p>
<select name="playlist" class="box">
   <option value="<?= $fecth_videos['playlist_id']; ?>" selected>--выберите плейлист</option>
   <?php
   ...
   <?php
      ...
      echo '<option value="" disabled>плейлистов пока не создано!</option>';
   }
   ?>
</select>
...
<input type="submit" value="обновить контент" name="update" class="btn">
<div class="flex-btn">
   <a href="view_content.php?get_id=<?= $video_id; ?>" class="option-btn">просмотреть контент</a>
   <input type="submit" value="удалить контент" name="delete_video" class="delete-btn">
</div>
</form>
<?php
      }
   }else{
      echo '<p class="empty">видео не найдено! <a href="add_content.php" class="btn" style="margin-top: 1.5rem;">добавить видео</a></p>';
   }
?>
</section>

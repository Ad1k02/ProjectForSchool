...
if(isset($_POST['like_content'])){
   ...
   if($user_id != ''){
      ...
      $message[] = 'Пожалуйста, сначала войдите!';
   }
   ...
   if($user_id != ''){
      ...
      if($select_content->rowCount() > 0){
         ...
         if($select_comment->rowCount() > 0){
            $message[] = 'комментарий уже добавлен!';
         }else{
            ...
            $message[] = 'новый комментарий добавлен!';
         }
...
<head>
   ...
   <!-- Ссылка на подключение Font Awesome CDN -->
...
<section class="edit-comment">
   ...
   <form action="" method="post">
      ...
      <textarea name="update_box" class="box" maxlength="1000" required placeholder="Пожалуйста, введите ваш комментарий" cols="30" rows="10"><?= $fetch_edit_comment['comment']; ?></textarea>
      <div class="flex">
         <a href="watch_video.php?get_id=<?= $get_id; ?>" class="inline-option-btn">Отменить редактирование</a>
         <input type="submit" value="Обновить" name="update_now" class="inline-btn">
      </div>
   </form>
</section>
<?php
...
<!-- Секция просмотра видео начинается здесь -->
...
<section class="watch-video">
   ...
   <div class="video-details">
      ...
      <div class="info">
         ...
         <p><i class="fas fa-heart"></i><span><?= $total_likes; ?> лайков</span></p>
      </div>
      <div class="tutor">
      ...
      <form action="" method="post" class="flex">
         ...
         <a href="playlist.php?get_id=<?= $fetch_content['playlist_id']; ?>" class="inline-btn">Просмотреть плейлист</a>
         <?php
            if($verify_likes->rowCount() > 0){
         ?>
         <button type="submit" name="like_content"><i class="fas fa-heart"></i><span>Лайкнуто</span></button>
         <?php
         }else{
         ?>
         <button type="submit" name="like_content"><i class="far fa-heart"></i><span>Лайк</span></button>
         <?php
            }
         ?>
      </form>
   ...
   <?php
         ...
         echo '<p class="empty">Видео пока не добавлено!</p>';
      }
   ?>

</section>

<!-- Секция просмотра видео заканчивается здесь -->

<!-- Начало секции комментариев -->

<section class="comments">

   <h1 class="heading">Добавить комментарий</h1>

   <form action="" method="post" class="add-comment">
      <input type="hidden" name="content_id" value="<?= $get_id; ?>">
      <textarea name="comment_box" required placeholder="Напишите свой комментарий..." maxlength="1000" cols="30" rows="10"></textarea>
      <input type="submit" value="Добавить комментарий" name="add_comment" class="inline-btn">
   </form>
   ...
   <div class="show-comments">
      ...
      <div class="box" style="<?php if($fetch_comment['user_id'] == $user_id){echo 'order:-1;';} ?>">
         ...
         <form action="" method="post" class="flex-btn">
            ...
            <button type="submit" name="edit_comment" class="inline-option-btn">Редактировать комментарий</button>
            <button type="submit" name="delete_comment" class="inline-delete-btn" onclick="return confirm('Удалить этот комментарий?');">Удалить комментарий</button>
         </form>
         <?php
         }
         ?>
      ...
      <?php
         ...
         echo '<p class="empty">Пока нет добавленных комментариев!</p>';
      }
      ?>
      </div>
